...
dataset="KITTI"
image_root=./data/my_data/images
label_root=./data/my_data/label_txt
data_root_dir=.
current_dir=`pwd`
...
length_imgs=`ls -l ${image_root}|grep '^-'|wc -l`
length_labels=`ls -l ${label_root}|grep '^-'|wc -l`
...
if [ ${length_imgs} != ${length_labels} ]; then
    ...
    j=0
	for img in `ls ${image_root_dir}|sort -h`
	do
		img_list[${j}]=${img}
		((j++))
	done
	...
	k=0
	for label in `ls ${label_root}|sort -h`
	do
		label_list[${k}]=${label}
		((k++))
	done
